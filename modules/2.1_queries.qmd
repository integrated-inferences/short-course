...
* the effect of $X$ on $Y$ when $M$ "is set to" a given value 

Say we thought that $X$ affects $Y$ through $M$. Going to school increased income via education. 

* we are more convinced that $X$ caused $Y$ if we observed that $M=1$
* we are expect that $X$ would *not* cause $Y$ if we set $M$ to $1$
