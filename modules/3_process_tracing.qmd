...
Simple insight: *If* you believe this model, then seeing $M$ should tell you something about a query regarding the $X$, $Y$ relationship.
...
make_model("X->Y <- Q") |> 
  plot_model(
      labels = c('X', "theta^Y", 'Y'),
      parse = TRUE,
      textcol  = "black", nodecol = "white",)

```

Key idea is that: *if* you believe this model, then observations about observable nodes (eg $X$, $Y$) should be informative about unobservable estimands ($\theta^Y$).
...
make_model("X-> M -> Y <- Q2; Q1 -> M") |> 
  plot_model(
      labels = c('X', "theta^Y", "theta^M", 'M', 'Y'),
      parse = TRUE,
      textcol  = "black", nodecol = "white",)
...
*If* you believe this model, then seeing $M$ should tell you something about the $\theta$s. 
...
# Example: Democratization in Malawi 


## A case level question
...
## New data on Malawi (within case data)
...
## Progress from prior beliefs about processes
...
## So: Process tracing is always theory dependent
...
# General strategy for process tracing with causal models
