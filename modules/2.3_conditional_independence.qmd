...
## Key insight linking DAGs to Bayes

The key insight that connects a DAG representation of a causal model and Bayesian updating is this:

* the DAG can tell you how to update on one feature given informmation on another
...
* If $K$ is *not* conditionally independent of $Q$ then $K$ *is* informative for $Q$.
* If $K$ *is* conditionally independent of $Q$ then $K$ is *not* for $Q$.

```{r, echo = FALSE}

make_model( "X-> M-> Y <- Q; K -> M") |> plot(x_coord = c(2, 2.5, 1, 1.5, 2 ), y_coord = c(2,1,2,1, 0))
```
